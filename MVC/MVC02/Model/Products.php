...
class Products{
    ...
    static function search(){
        ...
        $sql_count = "SELECT count(id) as total FROM products WHERE ID LIKE '%$search%' OR Name LIKE '%$search%' OR Price LIKE '%$search%' OR Description LIKE '%$search%' OR Producter LIKE '%$search%' ";
        $stmt_count = $conn->query($sql_count);
        $stmt_count->setFetchMode(PDO::FETCH_ASSOC);
        $row_count = $stmt_count->fetch();
        $total_records = $row_count['total'];
